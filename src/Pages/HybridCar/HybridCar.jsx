...
import './HybridCar.css'
import { Link } from 'react-router-dom';

const HybridCar = () => {
  const [car, setCar] = useState(carData.filter(item=> item.category==='hybrid'));
  const [input, setInput] = useState([]);
  const [loading,setLoading] = useState(true)

  const searchData = () => {
    const filtered = carData.filter(item => item.category==='hybrid').filter(item=>
    ...
    {loading ? (<h1>loading...</h1>
      ...
      <div className="container">
        <header className="header">
          <h1 className="title">Hibrid Avtomobillər</h1>
